...
export const ExperiencePositionItem = ({ position }: IProps) => {
  return (
    <div className="last:before:bg-card relative last:before:absolute last:before:h-full last:before:w-4">
      <div className="bg-card relative z-1 mb-1 flex items-center space-x-3">
          ...
          <div className="flex flex-wrap gap-1.5 pt-2 pl-7">
            {position.skills.map((skill, index) => {
              return (
                <div
                  key={index}
                  className="text-muted-foreground rounded-full border border-zinc-200 px-1.5 text-sm dark:border-zinc-700"
                >
                  {skill}
                </div>
              );
